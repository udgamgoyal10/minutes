...
function definitionRows(organizationId: number): DefinitionRow[] {
  return db.query<DefinitionRow, [number]>(
...
function overrideMap(organizationId: number): Map<number, OverrideRow> {
  const rows = db.query<OverrideRow, [number]>(
...
function userCustomMap(userId?: number): Map<string, CustomRow> {
  ...
  const rows = db.query<CustomRow, [number]>(
...
function toResolved(
  ...
  return {
    ...
    template_title: custom ? "My templates" : override ? "Organization override" : definition.owner_organization_id == null ? "Shared default" : "Organization section",
...
export function listSharedSections(): ResolvedSectionTemplate[] {
  return db.query<DefinitionRow, []>(
    `SELECT id, key, owner_organization_id, title, body_text,
            required_sources_json, required_variables_json, origin_template_id
     FROM section_template_definitions
     WHERE owner_organization_id IS NULL
     ORDER BY title COLLATE NOCASE, id`,
  ).all().map((definition) => toResolved(0, definition, undefined, undefined));
}
...
export function updateSharedSection(input: {
  ...
  const result = db.run(
    ...
    [
      ...
      JSON.stringify(input.requiredVariables ?? inferRequiredVariables(input.bodyText, input.title)),
      input.sectionTemplateId,
    ],
  );
  if (!result.changes) return null;
  return listSharedSections().find((section) => section.section_template_id === input.sectionTemplateId) ?? null;
}

export function listOrganizationSections(organizationId: number, userId?: number): ResolvedSectionTemplate[] {
  ...
  const sections = definitionRows(organizationId).map((definition) =>
    toResolved(organizationId, definition, overrides.get(definition.id), customs.get(definition.key))
  );
...
export function resolveOrganizationSection(
  ...
  const definition = db.query<DefinitionRow, [number, number]>(
    `SELECT id, key, owner_organization_id, title, body_text,
            required_sources_json, required_variables_json, origin_template_id
     FROM section_template_definitions
...
export function listMeetingStructures(organizationId: number, userId?: number, includeInactive = false): MeetingStructure[] {
  const rows = db.query<StructureRow, [number]>(
    `SELECT id, organization_id, base_template_id, slug, name, description, is_default, is_active
  ...
  return rows.map((row) => getMeetingStructure(row.id, userId)).filter((row): row is MeetingStructure => row != null);
}

export function getMeetingStructure(id: number, userId?: number): MeetingStructure | null {
  const row = db.query<StructureRow, [number]>(
    `SELECT id, organization_id, base_template_id, slug, name, description, is_default, is_active
     FROM meeting_structures WHERE id = ?`,
  ).get(id);
  if (!row) return null;
  const sectionRows = db.query<{ section_template_id: number }, [number]>(
  ...
  const sections = sectionRows
    .map((section) => resolveOrganizationSection(row.organization_id, section.section_template_id, userId))
...
export function resetOrganizationOverride(organizationId: number, sectionTemplateId: number): boolean {
  const result = db.run(
...
function uniqueSectionKey(organizationId: number, title: string): string {
  const org = db.query<{ slug: string }, [number]>("SELECT slug FROM organizations WHERE id = ?").get(organizationId);
  const base = title.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "section";
  let key = `${org?.slug ?? `org-${organizationId}`}-${base}`;
  let suffix = 2;
  while (db.query<{ id: number }, [string]>("SELECT id FROM section_template_definitions WHERE key = ?").get(key)) {
...
function slugify(value: string): string {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 70) || "meeting";
}
...
function uniqueStructureSlug(organizationId: number, name: string): string {
  ...
  ).get(organizationId, slug)) slug = `${base}-${suffix++}`;
  return slug;
}

function replaceStructureSections(structureId: number, organizationId: number, sectionTemplateIds: number[]): void {
  const uniqueIds = [...new Set(sectionTemplateIds)];
  for (const id of uniqueIds) {
    if (!resolveOrganizationSection(organizationId, id)) throw new Error(`Section template ${id} is not available to this organization`);
  }
...
export function createMeetingStructure(input: {
  ...
  if (!baseTemplateId || !db.query<{ id: number }, [number]>("SELECT id FROM meeting_templates WHERE id = ?").get(baseTemplateId)) {
  ...
  const result = db.run(
    `INSERT INTO meeting_structures
       (organization_id, base_template_id, slug, name, description, is_default)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [
      input.organizationId,
      baseTemplateId,
  ...
  const sectionIds = input.sectionTemplateIds ?? copied?.sections.map((section) => section.section_template_id).filter((value): value is number => value != null) ?? [];
...
export function updateMeetingStructure(input: {
  ...
  if (input.baseTemplateId && !db.query<{ id: number }, [number]>("SELECT id FROM meeting_templates WHERE id = ?").get(input.baseTemplateId)) {
    throw new Error("Document layout not found");
  }
  const transaction = db.transaction(() => {
    if (input.isDefault) db.run("UPDATE meeting_structures SET is_default = 0 WHERE organization_id = ?", [existing.organization_id]);
    db.run(
    ...
    if (input.sectionTemplateIds) replaceStructureSections(input.id, existing.organization_id, input.sectionTemplateIds);
  });
  transaction();
