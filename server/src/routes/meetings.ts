...
} from "../services/template-variables.ts";
...
function effectiveRequiredVariables(userId: number, sectionKey: string, base: string[]): string[] {
  ...
  return mergeVariables(base, userMappedVariables(userId, sectionKey)).filter(
...
function sectionTemplateKeysForUser(userId: number): Set<string> {
  ...
  const customRows = db
    .query<{ key: string }, [number]>("SELECT key FROM custom_section_templates WHERE user_id = ?")
    .all(userId);
...
function baseSectionKeysForToken(userId: number, token: string): string[] {
  ...
  for (const row of rows) {
    ...
    for (const section of parsed.sections) {
      if (inferRequiredVariables(section.bodyText, section.title).includes(token))
        keys.add(section.key);
    }
  }
  const customRows = db
    .query<CustomTemplateRow, [number]>(
...
function saveVariableMapping(
  ...
  db.run("DELETE FROM user_template_variable_mappings WHERE user_id = ? AND token = ?", [
    userId,
    token,
  ]);
  db.run("DELETE FROM user_template_variable_mapping_exclusions WHERE user_id = ? AND token = ?", [
    userId,
    token,
  ]);
...
function catalogForUser(userId: number, organizationId?: number) {
  const organizationSlug = organizationId
    ...
    : undefined;
  const base = setupVariableCatalog(organizationSlug);
...
function baseRequiredVariablesForSection(userId: number, sectionKey: string): string[] | null {
  const custom = db
    .query<CustomTemplateRow, [number, string]>(
...
function setSectionVariableLinks(userId: number, sectionKey: string, tokens: string[]): boolean {
  const desired = [
    ...new Set(
      tokens.filter((x) => typeof x === "string" && x.trim().length > 0).map(canonicalToken),
    ),
  ];
  const custom = db
    .query<CustomTemplateRow, [number, string]>(
  ...
  if (custom) {
    db.run(
      "UPDATE custom_section_templates SET required_variables_json = ? WHERE id = ? AND user_id = ?",
      [JSON.stringify(desired), custom.id, userId],
    );
    db.run("DELETE FROM user_template_variable_mappings WHERE user_id = ? AND section_key = ?", [
      userId,
      sectionKey,
    ]);
    db.run(
      "DELETE FROM user_template_variable_mapping_exclusions WHERE user_id = ? AND section_key = ?",
      [userId, sectionKey],
    );
    return true;
  }
...
function addNewVariablesForSection(
  userId: number,
  sectionKey: string,
  variables: unknown,
): string[] {
  ...
  for (const item of variables) {
    ...
    const kind =
      typeof item === "object" && item && (item as { kind?: string }).kind === "date"
        ? "date"
        : "text";
...
function customToSection(row: CustomTemplateRow, userId?: number, canEdit = true) {
  ...
  return {
    ...
    template_title:
      row.user_id === userId
        ? "My templates"
        : `Shared templates${row.owner_email ? ` (${row.owner_email})` : ""}`,
...
r.get("/template-variables", (c) => {
  ...
  const organizationId = Number(c.req.query("organization_id")) || undefined;
  return c.json({ variables: catalogForUser(user.id, organizationId) });
});
...
r.post("/template-variables", async (c) => {
  ...
  if (!sectionKeys)
    return c.json(
      { error: "template variable must be mapped to at least one valid section template" },
      400,
    );
  saveVariableMapping(
    user.id,
...
r.delete("/template-variables/:token", (c) => {
  ...
  const customRows = db
    .query<{ id: number; required_variables_json: string | null }, [number]>(
...
r.patch("/section-templates/:key/variables", async (c) => {
  ...
  if (!sectionTemplateKeysForUser(user.id).has(key))
    return c.json({ error: "section template not found" }, 404);
  const body = (await c.req.json().catch(() => ({}))) as Partial<{
    required_variables: string[];
  ...
  if (!setSectionVariableLinks(user.id, key, tokens))
    return c.json({ error: "section template not found" }, 404);
...
r.patch("/section-templates/:key", async (c) => {
  ...
  const row = db
    ...
    .get(user.id, key);
  return c.json({
    template: row ? customToSection(row, user.id) : null,
    variables: catalogForUser(user.id),
  });
});
...
r.patch("/section-templates/custom/:id", async (c) => {
  ...
  const existing = isAdminRole(user.role)
    ? db
        ...
        .get(id)
    : db
        .query<CustomTemplateRow, [number, number]>(
...
r.post("/variable-values", async (c) => {
  ...
  const entries =
    body.entries ??
    (body.token ? [{ token: body.token, value: body.value ?? "", gender: body.gender }] : []);
  const insert = db.prepare(
...
r.patch("/variable-values", async (c) => {
  ...
  if (!token || !oldValue || !value)
    return c.json({ error: "token, old_value and value required" }, 400);
  if (Object.prototype.hasOwnProperty.call(body, "gender")) {
    db.run(
      "UPDATE template_variable_values SET value = ?, gender = ? WHERE user_id = ? AND token = ? AND value = ?",
      [value, normalizeGender(body.gender), user.id, token, oldValue],
    );
  } else {
    db.run(
      "UPDATE template_variable_values SET value = ? WHERE user_id = ? AND token = ? AND value = ?",
      [value, user.id, token, oldValue],
    );
  }
  return c.json(variableValuesForUser(user.id));
});
...
r.delete("/variable-values", async (c) => {
  ...
  db.run("DELETE FROM template_variable_values WHERE user_id = ? AND token = ? AND value = ?", [
    user.id,
    token,
    value,
  ]);
...
r.get("/meetings", (c) => {
  ...
  const rows = isAdminRole(user.role)
    ? db
        .query<MeetingRow & { owner_email: string }, []>(
        ...
        .all()
    : db
        .query<MeetingRow & { owner_email: string }, [number]>(
          `SELECT m.*, u.email AS owner_email
         FROM meetings m
         JOIN users u ON u.id = m.user_id
         WHERE m.user_id = ?
...
r.post("/meetings", async (c) => {
  ...
  const tpl = db
    .query<TemplateRow, [number]>("SELECT * FROM meeting_templates WHERE id = ?")
    .get(templateId);
  ...
  const label =
    body?.label?.trim() || `${structure?.name ?? tpl.title} — ${new Date().toLocaleDateString()}`;
  const parsed = JSON.parse(tpl.parsed_json) as ParsedTemplate;
  const legacyCatalog = new Map(
    sectionTemplateCatalog(user.id).map((section) => [section.key, section]),
  );
  const resolvedSections =
    ...
    parsed.sections.map((section) => {
      ...
      return {
        ...
        required_sources:
          catalogSection?.required_sources ?? inferRequiredSources(section.title, section.bodyText),
        required_variables:
          catalogSection?.required_variables ??
          inferRequiredVariables(section.bodyText, section.title),
      };
    });
  ...
  const tx = db.transaction(() => {
    ...
    resolvedSections.forEach((section, index) => {
      const requiredVariables = effectiveRequiredVariables(
        user.id,
        section.key,
        section.required_variables,
      );
      insertSec.run(
        meetingId,
        section.key,
  ...
  const meeting = db
    .query<MeetingRow, [number]>("SELECT * FROM meetings WHERE id = ?")
    .get(meetingId);
...
function getMeetingRow(id: number, user: { id: number; role: string }): MeetingRow | null {
  if (isAdminRole(user.role)) {
    return db.query<MeetingRow, [number]>("SELECT * FROM meetings WHERE id = ?").get(id) ?? null;
  }
  return (
    db
      .query<MeetingRow, [number, number]>("SELECT * FROM meetings WHERE id = ? AND user_id = ?")
      .get(id, user.id) ?? null
  );
}
...
function rowToMeeting(row: MeetingRow) {
  return {
    ...
    owner_email:
      "owner_email" in row
        ? ((row as MeetingRow & { owner_email?: string }).owner_email ?? null)
        : null,
    label: row.label,
