...
import { FileCog, FileText, LogOut, Plus, Users } from "lucide-react";
        ...
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 text-sm">
            ...
            {(user?.role === "admin" || user?.role === "super_admin") && (
              <Link to="/admin/templates" className="flex items-center gap-1 text-slate-600 hover:text-slate-900">
                <FileCog className="size-4" /> Templates
              </Link>
            )}
